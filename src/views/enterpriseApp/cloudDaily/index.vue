...
export default {
  ...
  data() {
    return {
      ...
      appName: '云日报',
      appIcon: window.unescape(this.$route.query.appIcon),
      activeSelTab: '1',
      activeTab: '1',
      tabListData: [
        {
  ...
  methods: {
    ...
    setSelectTab(item) {
      ...
      switch (item.tabId) {
        case '1':
          that.changeRoute(`dailyRecord?appIcon=${that.$route.query.appIcon}`);
          break;
        case '2':
        case '21':
          that.changeRoute(`systemReport?appIcon=${that.$route.query.appIcon}`);
          break;
        case '211':
          that.changeRoute(`systemReport?appIcon=${that.$route.query.appIcon}`);
          break;
        case '212':
          that.changeRoute(`customReport?appIcon=${that.$route.query.appIcon}`);
          break;
        case '22':
          that.changeRoute(`taskAssign?appIcon=${that.$route.query.appIcon}`);
          break;
      }
    },
