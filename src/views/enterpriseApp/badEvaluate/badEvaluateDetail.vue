<template>
  <div class="my-customer-wrap common-set-wrap">
    ...
    <div class="daily-set-content boder-box">
      ...
      <el-table class="select-table" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" :height="tableH" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="left">
          <template slot="header" slot-scope="scope">
            <span>{{ !!scope.row ? '完成备注' : '放弃备注' }}</span>
          </template>
          <template slot-scope="scope">
            <el-popover placement="top-start" title="" width="200" trigger="hover" :content="scope.row.reason">
              <i class="iconfont icon-beizhu" style=""></i>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column label="完成备注">
          <template slot-scope="scope">
            <el-popover v-if="scope.row.isCompleted == '已完成'" placement="top-start" title="" width="200" trigger="hover" :content="scope.row.isCompleted == '已完成' ? scope.row.reason : ''">
        ...
        <el-table-column label="放弃备注">
          <template slot-scope="scope">
            <el-popover v-if="scope.row.isCompleted == '未完成'" placement="top-start" title="" width="200" trigger="hover" :content="scope.row.isCompleted == '已完成' ? '' : scope.row.reason">
              <i slot="reference" class="iconfont icon-beizhu" style="font-size: 40px;"></i>
            </el-popover>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    postMultDel(obj) {
      let that = this;
      let para = {
        taskType: 0, // 0 不良评价，1 话务任务
        storeId: that.conditionObj.storeId,
        taskIds: that.multipleSelection.map(ele => ele.taskIds) || '', // 门店分组,数组
