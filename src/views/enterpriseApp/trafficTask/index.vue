...
export default {
      ...
      tabListData: [
        {
          tabId: '1',
          tabName: '话务任务记录',
          icon: 'icon-tonghuahuawuliang',
          onlyIconActive: false
        },
        {
          tabId: '2',
          tabName: '话务任务设置',
          icon: 'icon-huawujinengzupeizhi',
          onlyIconActive: false
        },
        {
          tabId: '3',
  ...
  methods: {
    ...
    setSelectTab(item) {
      ...
      switch (item.tabId) {
        case '1':
          that.changeRoute(`taskRecord?appIcon=${that.$route.query.appIcon}`);
          break;
        case '2':
          that.changeRoute(`trafficTaskSet?appIcon=${that.$route.query.appIcon}`);
          break;
        case '3':
          that.changeRoute(`badEvaluateSet?appIcon=${that.$route.query.appIcon}`);
          break;
      }
    },
